...
class Doc extends DocBase {
  ...
  get baseURL () {
    return 'https://insta.js.org'
  }

  get baseDocsURL () {
    if (!this.baseURL) return null
    const repo = ['insta.js'].includes(this.repo) ? 'main' : this.repo
    return `${this.baseURL}/#/docs/${repo}/${this.branch}`
  }

  get icon () {
    if (!this.baseURL) return null
    return `${this.baseURL}/static/logo-square.png`
  }
  ...
  static getRepoURL (id) {
    ...
    return `https://github.com/Androz2091/${project}/blob/${branch}/`
  }
